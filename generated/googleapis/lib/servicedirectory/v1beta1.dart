...
///       - [ProjectsLocationsNamespacesWorkloadsResource]
library;
...
class ProjectsLocationsNamespacesResource {
  ...
  ProjectsLocationsNamespacesWorkloadsResource get workloads =>
      ProjectsLocationsNamespacesWorkloadsResource(_requester);
  ...
  async.Future<Namespace> create(
    ...
    final url_ = 'v1beta1/' + core.Uri.encodeFull('$parent') + '/namespaces';
  ...
  /// string syntax: ` ()` * `` can be `name`, `labels.` for map field, or
  ...
  /// namespaces, it returns no results * `attributes.managed_registration=true`
  /// returns namespaces that are managed by a GCP product or service For more
...
class ProjectsLocationsNamespacesServicesResource {
  ...
  async.Future<Service> create(
    ...
    final url_ = 'v1beta1/' + core.Uri.encodeFull('$parent') + '/services';
  ...
  ///
  /// [filter] - Optional. The filter to list results by. General `filter`
  /// string syntax: ` ()` * `` can be `name` or `metadata.` for map field * ``
  ...
  /// `owner` in metadata key but value is not `sd` AND have key/value `foo=bar`
  /// * `doesnotexist.foo=bar` returns an empty list. Note that service doesn't
  /// have a field called "doesnotexist". Since the filter does not match any
  /// services, it returns no results * `attributes.managed_registration=true`
  /// returns services that are managed by a GCP product or service For more
  /// information about filtering, see [API Filtering](https://aip.dev/160).
  ///
  /// [orderBy] - Optional. The order to list results by. General `order_by`
  /// string syntax: ` () (,)` * `` allows value: `name` * `` ascending or
  ...
  async.Future<ResolveServiceResponse> resolve(
    ...
    final url_ = 'v1beta1/' + core.Uri.encodeFull('$name') + ':resolve';
...
class ProjectsLocationsNamespacesServicesEndpointsResource {
  ...
  ///
  /// [filter] - Optional. The filter to list results by. General `filter`
  /// string syntax: ` ()` * `` can be `name`, `address`, `port`, `metadata.`
  /// for map field, or `attributes.` for attributes field * `` can be `<`, `>`,
  /// `<=`, `>=`, `!=`, `=`, `:`. Of which `:` means `HAS`, and is roughly the
  /// same as `=` * `` must be the same data type as field * `` can be `AND`,
  /// `OR`, `NOT` Examples of valid filters: * `metadata.owner` returns
...
class ProjectsLocationsNamespacesWorkloadsResource {
  final commons.ApiRequester _requester;

  ProjectsLocationsNamespacesWorkloadsResource(commons.ApiRequester client)
    : _requester = client;
...
class Endpoint {
  ...
  /// metadata dictionary may contain up to 512 characters, spread accoss all
  /// key-value pairs. Metadata that goes beyond this limit are rejected * Valid
  /// metadata keys have two segments: an optional prefix and name, separated by
  /// a slash (/). The name segment is required and must be 63 characters or
  ...
  /// in total, followed by a slash (/). Metadata that fails to meet these
  /// requirements are rejected Note: This field is equivalent to the
...
class ResolveServiceRequest {
  ...
  /// `!=`, `=`, `:`. Of which `:` means `HAS`, and is roughly the same as `=` *
  /// `` must be the same data type as field * `` can be `AND`, `OR`, `NOT`
  /// Examples of valid filters: * `metadata.owner` returns endpoints that have
  /// a annotation with the key `owner`, this is the same as `metadata:owner` *
...
class Service {
  ...
  ///
  /// This data can be consumed by service clients. Restrictions: * The entire
  /// metadata dictionary may contain up to 2000 characters, spread accoss all
  /// key-value pairs. Metadata that goes beyond this limit are rejected * Valid
  /// metadata keys have two segments: an optional prefix and name, separated by
  /// a slash (/). The name segment is required and must be 63 characters or
  /// less, beginning and ending with an alphanumeric character (\[a-z0-9A-Z\])
