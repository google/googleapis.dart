...
class GoogleCloudEventarcV1PipelineDestinationHttpEndpoint {
  /// The CEL expression used to construct a new HTTP request to be sent to the
  /// final destination.
  ///
  /// The result of the CEL expression must be a map of key/value pairs such
  /// that: - If a map named `headers` exists on the result of the expression,
  /// then its key/value pairs are directly mapped to the HTTP request headers.
  /// The headers values are constructed from the corresponding value type’s
  ...
  /// a JSON string. If the body field doesn’t exist then the resulting HTTP
  /// request will be without a body. - Any other fields in the resulting
  /// expression will be ignored. The CEL expression may access the incoming
  /// CloudEvent message in its definition, as follows: - The `data` field of
  /// the incoming CloudEvent message can be accessed using the `message.data`
  /// value. - Each attribute of the incoming CloudEvent message can be accessed
  /// using the `message.` value, where is replaced with the name of the
  /// attribute. Headers added to the request by previous filters in the chain
  /// can be accessed in the CEL expression using the `headers` variable. The
  /// `headers` variable defines a map of key/value pairs corresponding to the
  /// HTTP headers added by previous mediation steps and not the headers present
  /// on the original incoming request. For example, the following CEL
  /// expression can be used to construct a Headers-only HTTP request by adding
  /// an additional header to the headers added by previous mediations in the
  /// Pipeline: ``` {"headers": headers.merge({"new-header-key":
  ...
  /// string - toJsonString: map.toJsonString() -\> string - Converts a CelValue
  /// to a JSON string - merge: map1.merge(map2) -\> map3 - Merges the passed
  /// CEL map with the existing CEL map the function is applied to. - If the
  /// same key exists in both maps, if the key's value is type map both maps are
  /// merged else the value from the passed map is used. - toMap:
  /// list(map).toMap() -\> map - Converts a CEL list of CEL maps to a single
  /// CEL map - toDestinationPayloadFormat():
  ...
  /// data to a JSON string it can be chained with the toJsonString function. If
  /// a binding is not specified here, by default the message is treated as a
  ...
  /// Pipeline.Destination.output_payload_format and maps it to the body field
  ...
  /// will treat the data field of the message as opaque binary data and attach
  /// it to the request body as bytes. In this case the Content-type header will
  /// be set to the value of the datacontenttype attribute set on the incoming
  /// CloudEvent message if present and the `application/octet-stream` MIME type
  /// otherwise. The Pipeline expects that the content of the message will
  /// adhere to the standard CloudEvent format. If it doesn’t then the outgoing
...
class GoogleCloudEventarcV1PipelineMediationTransformation {
  /// The template to apply to transform messages.
  ///
  /// Optional.
...
class GoogleCloudEventarcV1PipelineRetryPolicy {
  ...
  ///
  /// The value must be between 0 and 600. The default value for this field is
  /// 60.
  ///
  /// Optional.
...
class ListChannelConnectionsResponse {
  ...
  ListChannelConnectionsResponse.fromJson(core.Map json_)
      : this(
          channelConnections: (json_['channelConnections'] as core.List?)
