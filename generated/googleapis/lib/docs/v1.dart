...
class DocumentsResource {
  ...
  ///
  /// [includeTabsContent] - Whether to populate the `Document.tabs` field
  /// instead of the text content fields like `body` and `documentStyle` on
  /// `Document`. - When `true`: Document content populates in the
  /// `Document.tabs` field instead of the text content fields in `Document`. -
  /// When `false`: The content of the document's first tab populates the
  /// content fields in `Document` excluding `Document.tabs`. If a document has
  /// only one tab, then that tab is used to populate the document content.
  /// `Document.tabs` will be empty. If you use a field mask that references the
  /// `Document.tabs` field (or any subfield), the API implicitly treats the
...
class UpdateNamedStyleRequest {
  ...
  /// to bold, set `fields` to include `"text_style"` and `"text_style.bold"`.
  /// To update the paragraph style's alignment property, set `fields` to
  /// include `"paragraph_style"` and `"paragraph_style.alignment"`. To reset a
  /// property to its default value, include its field name in the field mask
  /// but leave the field itself unset. Specifying `"text_style"` or
  /// `"paragraph_style"` with an empty TextStyle or ParagraphStyle will reset
